...
/**
 * Make a button that open a Bootstrap4 Modal with html content.
 *
...
class ModalButton extends Widget
{
    ...
    /*
     * Available events:
     * - `onFormSuccess`: fire after the form in the modal window is successfully submitted and before closing the modal window. This is only usable if the modal window has a form.
     * - `onOpenModal`: fire when the modal window is opened.
     */
	public $events = [];

	/**
	 * @var string the ID of the HTML content. This is used to attach an event to the modal window.
	 */
	public $contentId = 'modal-window';
	...
	public function init()
	{
        ...
        if ($this->headerLabel === null) {
            throw new InvalidConfigException(Yii::t('mlogin', 'The "headerLabel" property must be set.'));
        }

        if (isset($this->events['onFormSuccess'])) {
            $userEvent = $this->events['onFormSuccess'];
